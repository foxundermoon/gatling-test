...
/**
  ...
  */
class VerboseLogFileReqConfig(val users:Int) {
...
//    .baseURL("http://127.0.0.1:9600")
    ...
    .contentTypeHeader("multipart/form-data; boundary=------------------------1bb290f73703a153")
  ...
  val scn = scenario("VerboseSmallLogSimulation")
    .feed(headerFeeder)
    .exec(http("request_0")
      .post("/v1/log/file/upload")
      .headers(Map("Charsert" -> "${Charsert}", "X-EXTRA" -> "${X-EXTRA}"))
      .body(RawFileBody("RecordedSimulation_v_request.txt"))
      .check(status.is(202))
    )
}
